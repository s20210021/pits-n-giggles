...
document.getElementById("best-lap-th").addEventListener("click", function () {
    ...
    showToast(t('frontend.toast.best_lap_changed', { format: (g_pref_bestLapAbsoluteFormat ? "Absolute" : "Relative") }));
    savePreferences();
});
...
document.getElementById("last-lap-th").addEventListener("click", function () {
    ...
    showToast(t('frontend.toast.last_lap_changed', { format: (g_pref_lastLapAbsoluteFormat ? "Absolute" : "Relative") }));
    savePreferences();
});
...
document.getElementById("wear-prediction-th").addEventListener("click", function () {
    ...
    showToast(t('frontend.toast.tyre_wear_changed', { format: (g_pref_tyreWearAverageFormat ? "Average" : "Max") }));
    savePreferences();
});
...
document.getElementById("fuel-info-th").addEventListener("click", function () {
    ...
    showToast(t('frontend.toast.fuel_target_changed', { format: (g_pref_fuelTargetAverageFormat ?
...
document.getElementById('tt-vmax-th').addEventListener('click', function (e) {
    ...
    showToast(t('frontend.toast.speed_changed', { format: (g_pref_speedUnitMetric ? "km/h" : "mph") }));
    savePreferences();
});
